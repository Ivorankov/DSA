namespace Problem_03.BiDictionary
{
    using System;
    using System.Collections.Generic;
    using Wintellect.PowerCollections;

    public class BiDictionary<TK1, TK2, TV>
    {
        private Dictionary<TK1, Guid> key1Identifiers;
        private Dictionary<TK2, Guid> key2Identifiers;
        private MultiDictionary<Guid, TV> values;

        public BiDictionary()
        {
            this.key1Identifiers = new Dictionary<TK1, Guid>();
            this.key2Identifiers = new Dictionary<TK2, Guid>();
            this.values = new MultiDictionary<Guid, TV>(true);
        }

        public void Add(TK1 key1, TK2 key2, TV value)
        {
            var id = this.GenerateId();
            if (this.key1Identifiers.ContainsKey(key1) && this.key2Identifiers.ContainsKey(key2))
            {
                var id1 = this.key1Identifiers[key1];
                var id2 = this.key2Identifiers[key2];
                if (id1 == id2)
                {
                    id = id1;
                    this.values.Add(id, value);
                }
            }
            else
            {
                this.key1Identifiers.Add(key1, id);
                this.key2Identifiers.Add(key2, id);
                this.values.Add(id, value);
            }
        }

        public ICollection<TV> Find(TK1 key)
        {
            if (this.key1Identifiers.ContainsKey(key))
            {
                var id = this.key1Identifiers[key];

                return this.values[id];
            }
            else
            {
                return null;
            }
        }

        public ICollection<TV> Find(TK2 key)
        {
            if (this.key2Identifiers.ContainsKey(key))
            {
                var id = this.key2Identifiers[key];

                return this.values[id];
            }
            else
            {
                return null;
            }
        }

        public ICollection<TV> Find(TK1 key1, TK2 key2)
        {
            if (this.key1Identifiers.ContainsKey(key1) && this.key2Identifiers.ContainsKey(key2))
            {
                var id1 = this.key1Identifiers[key1];
                var id2 = this.key2Identifiers[key2];
                if (id1 == id2)
                {
                    return this.values[id1];
                }
            }

            return null;
        }

        private Guid GenerateId()
        {
            var guid = Guid.NewGuid();
            while (this.values.ContainsKey(guid))
            {
                guid = Guid.NewGuid();
            }

            return guid;
        }
    }
}
